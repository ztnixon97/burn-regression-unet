...
impl<B: Backend> UNetPlusPlus<B> {
    pub fn init(in_channels: usize, out_channels: usize, device: &B::Device) -> Self {
        ...
        let up1 = ConvTranspose2dConfig::new([filters[4], filters[4]], [2, 2])
            .with_stride([2, 2])
            .init(device);
        let upconv1 = NestedConvConfig::new(filters[4] + filters[3], filters[3] /2 , filters[3]).init(device);  // mid_channels = filters[3] / 2

        let up2 = ConvTranspose2dConfig::new([filters[3], filters[3]], [2, 2])
            .with_stride([2, 2])
            .init(device);
        let upconv2 = NestedConvConfig::new(filters[3] *2 + filters[2], filters[2] / 2, filters[2]).init(device);  // mid_channels = filters[2] / 2

        let up3 = ConvTranspose2dConfig::new([filters[2], filters[2]], [2, 2])
            .with_stride([2, 2])
            .init(device);
        let upconv3 = NestedConvConfig::new(filters[2] * 2 + filters[3] + filters[1], filters[1] / 2, filters[1]).init(device);  // mid_channels = filters[1] / 2

        let up4 = ConvTranspose2dConfig::new([filters[1], filters[1]], [2, 2])
            .with_stride([2, 2])
            .init(device);
        let upconv4 = NestedConvConfig::new(filters[1] *2 + filters[2] + filters[3] + filters[0], filters[0] / 2, filters[0]).init(device);  // mid_channels = filters[0] / 2
